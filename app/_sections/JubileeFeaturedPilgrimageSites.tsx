            ...
            {allFeaturedSites.map((site, key) => {
              return (
                <Link
                  ...
                  title={site.name}
                  key={key}
                  className="relative group col-span-4 lg:col-span-3 flex flex-col justify-between items-center overflow-hidden aspect-document rounded-lg bg-stone-100 hover:bg-stone-200/50 hover:shadow-xl hover:ring-4 ring-green-500/50 transition duration-300"
                >
                  <div className="absolute z-20 bottom-0 left-0 w-full text-xs uppercase font-bold tracking-wider font-sans text-center text-white bg-stone-700 px-4 py-2">
                    {site.state}
                  </div>
                  <Image
                    src={site.image}
